...
var manager streamsManager = streamsManager{
	isStream:       false,
...
func HandleStreaming(w http.ResponseWriter, r *http.Request) {
	...
	// check stream was exist or not
	if hasStream() {
		manager.stopStreamChan <- true
		time.Sleep(5 * time.Second)
	}

	respSDPChan := make(chan webrtc.SessionDescription)
	go streaming(offer, respSDPChan)
	remoteSDP, ok := <-respSDPChan
	if !ok {
...
func streaming(offer webrtc.SessionDescription, remoteSDPChan chan webrtc.SessionDescription) {
	logger := rolling.New()
	defer logger.Error("streaming ended!")

	defer close(remoteSDPChan)
	...
	defer func() {
		if cErr := peerConnection.Close(); cErr != nil {
			logger.Error("peerConnection.Close failed", zap.Error(cErr))
		}
	}()
	...
	stopRoutinesChan := make(chan bool, 1)
	...
	go func() {
		...
		for ; true; <-ticker.C {
			if len(stopRoutinesChan) == cap(stopRoutinesChan) {
	...
	// remoteSDP := Encode(*peerConnection.LocalDescription())
	// fmt.Println(remoteSDP)
	// clipboard.WriteAll(remoteSDP)
	remoteSDPChan <- *peerConnection.LocalDescription()

	<-manager.stopStreamChan
}

func hasStream() bool {
	manager.mtx.Lock()
	defer manager.mtx.Unlock()
	return manager.isStream
}

func setStream() {
	manager.mtx.Lock()
	defer manager.mtx.Unlock()
	manager.isStream = true
}
